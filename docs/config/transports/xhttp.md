...
以下选项用于改变 padding、会话标识和上行数据在 HTTP 请求中的表现形式，主要用于兼容会按固定特征过滤 XHTTP 的 CDN / WAF。客户端与服务端必须使用相互兼容的配置；它们只能减少固定特征，不能保证流量不可识别。
...
是否启用可配置的 padding 外观。默认值为 `false`，保持旧版 `Referer: ...?x_padding=XXX...` 行为以兼容旧客户端。启用后，下列 `xPaddingKey`、`xPaddingHeader`、`xPaddingPlacement` 和 `xPaddingMethod` 才用于新的外观。

> `xPaddingKey`: string

承载 padding 的 query 参数或 Cookie 名称。默认值为 `"x_padding"`。

> `xPaddingHeader`: string

`xPaddingPlacement` 为 `"queryInHeader"` 时，承载含 padding URL 的 HTTP 头名称；为 `"header"` 时则直接承载 padding。默认值为 `"X-Padding"`，旧模式的客户端请求仍使用 `Referer`。

> `xPaddingPlacement`: "queryInHeader" | "query" | "header" | "cookie"

padding 的位置。默认值为 `"queryInHeader"`：将 padding 作为 URL query，再把该 URL 放进指定 HTTP 头。其它值分别直接放进请求 query、HTTP 头或 Cookie。

> `xPaddingMethod`: "repeat-x" | "tokenish"

padding 内容生成方式。`"repeat-x"` 重复字符 `X`，默认且兼容旧行为；`"tokenish"` 生成类似普通令牌的随机字符，并按 HTTP 头压缩后的长度进行修正。

> `uplinkHTTPMethod`: string

上行请求使用的 HTTP 方法，不区分大小写，内部会转为大写。默认值为 `"POST"`。可按中间盒能力使用 `PUT`、`PATCH`、`GET` 或其它方法；`GET` 仅可用于 `packet-up`。这不是自动探测或轮换功能，所选方法必须能通过链路上的 CDN 和反向代理。

> `sessionPlacement`: "path" | "query" | "header" | "cookie"

会话 ID 的位置，默认值为 `"path"`。非 path 模式可用 `sessionKey` 自定义字段名；未填写时，query / cookie 默认使用 `x_session`，header 默认使用 `X-Session`。

> `seqPlacement`: "path" | "query" | "header" | "cookie"

`packet-up` 分包序号的位置，默认值为 `"path"`。非 path 模式可用 `seqKey` 自定义字段名；未填写时，query / cookie 默认使用 `x_seq`，header 默认使用 `X-Seq`。

> `uplinkDataPlacement`: "auto" | "body" | "header" | "cookie"

上行数据的位置，默认值为 `"auto"`；普通上行使用 body，服务端可自动接受支持的位置。`"header"` 和 `"cookie"` 仅可用于 `packet-up`，数据会以 Base64 编码并分片。这主要用于只能通过 GET 或禁止请求体的特殊链路，不建议作为常规配置。

> `uplinkDataKey`: string

Header 或 Cookie 承载上行分片时使用的基础名称。未填写时，Cookie 默认使用 `x_data`，Header / auto 默认使用 `X-Data`。

> `uplinkChunkSize`: number | string

上行数据放在 Header / Cookie 时，每个 Base64 分片的大小。支持范围值以随机化。它只限制单个分片，单次上行请求的总数据量仍由 `scMaxEachPostBytes` 控制。

> `serverMaxHeaderBytes`: number

仅服务端。调整 XHTTP HTTP 服务端接受的请求头总大小上限。`0` 使用实现默认值；负数无效。只有在确实控制整个链路时才应增大，因为 CDN 或反向代理可能仍使用更小的上限。

::: warning
Header / Cookie 上行会产生 Base64 膨胀并受单个字段、请求头总大小以及 CDN 限制。遇到 HTTP `431` 时，应同时调小 `uplinkChunkSize` 和 `scMaxEachPostBytes`；请求变多后，可能还要相应调大服务端 `scMaxBufferedPosts`。不要仅增大 `serverMaxHeaderBytes`，因为它无法改变 CDN 或反向代理的限制。
:::
...
客户端每个分包上行请求最多携带多少数据（字节）。字段名为兼容旧版而保留了 `Post`；使用 PUT、PATCH 或 GET 时同样生效。该值应小于 CDN 等 HTTP 中间盒所允许的最大值，服务端也会拒绝超限请求。
...
1. **客户端**默认以 `POST /yourpath/sameUUID/seq` 发送上行数据
   ...
   - UUID 和 seq 默认放在 path；启用 HTTP 外观选项后，也可放在 query、Header 或 Cookie
